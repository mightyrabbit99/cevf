...
#define _cevf_mod_add_producer(_thname, _stack_size) do { \
  _cevf_ret = 0; \
...
#define cevf_mod_add_producer(_thname, _stack_size) do { \
  _cevf_mod_add_producer(_thname, _stack_size); \
  if (_cevf_ret == -1) return; \
} while (0)
#define _cevf_mod_add_consumer(_ev_typ, _handler) do { \
  _cevf_ret = 0; \
...
#define cevf_mod_add_consumer(_ev_typ, _handler) do { \
